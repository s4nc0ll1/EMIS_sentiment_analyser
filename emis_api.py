        ...
        try:
            ...
            for country in data.get("data", {}).get("items", []):
                if "name" in country and "countryCode" in country:
                    self._country_lookup[country["name"].lower()] = country[
                        "countryCode"
                    ]
...
class SearchValidator:
    ...
    def validate_search_parameters(params: SearchParameters) -> None:
        ...
        if params.limit > 1000:
            logger.warning(f"Large limit specified: {params.limit}")
...
class CompanyMatcher:
    ...
    def match_company(
        self, company_name: str, country_name: Optional[str] = None
        ...
        try:
            response = self.http_client.get(
                APIEndpoint.COMPANIES_MATCH.value, params=params
            )
    ...
    def _process_company_match_response(
        self, response: Dict[str, Any], company_name: str
    ) -> Optional[CompanyMatch]:
        if (
            not response
            or "data" not in response
            or not (items := response["data"].get("items"))
        ):
...
class EmisDocuments:
    ...
    def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if not self.api_key:
            raise ValidationError("EMIS API key not set.")
    ...
    def _resolve_search_parameters(self, params: SearchParameters) -> Dict[str, Any]:
        ...
        country_code = (
            self.lookup_manager.get_country_code(params.country_name)
        ...
        company_match = (
            self.company_matcher.match_company(params.company_name, params.country_name)
        ...
        industry_code = (
            self.lookup_manager.get_industry_code(params.industry_name)
    ...
    def _search_documents(self, resolved_params: Dict[str, Any]) -> List[Document]:
        ...
        if not any(
            field in api_call_params
            for field in ["country", "company_ids", "keyword", "industry"]
        ):
        ...
        try:
            response = self.http_client.get(
                APIEndpoint.DOCUMENTS_SEARCH.value, params=api_call_params
            )
    ...
    def _build_search_result(
        self, documents: List[Document], resolved_params: Dict[str, Any]
    ) -> Dict[str, Any]:
