...
class UIComponents:
    ...
    def render_search_filters() -> SearchParameters:
        ...
        selected_country = st.sidebar.selectbox("Country Name (Optional)", options=country_list, index=0)
        company_input = st.sidebar.text_input("Company Name (Optional)")
        selected_industry = st.sidebar.selectbox("Industry (Optional)", options=industry_list, index=0)
    ...
    def render_topics_industries_charts(cls, documents: List[Dict[str, Any]]) -> None:
        ...
        with col1:
            ...
            if not topic_df.empty:
                top_n = 15
                fig = px.bar(topic_df.head(top_n).sort_values('count', ascending=True), x='count', y='name',
                             orientation='h', labels={'name': 'Topic', 'count': 'Doc Count'},
                             title=f"Top {min(top_n, len(topic_df))} Topics")
                fig.update_layout(yaxis={'categoryorder':'total ascending'}, title_x=0.5, height=400)
        ...
        with col2:
            ...
            if not industry_df.empty:
                top_n = 15
                fig = px.bar(industry_df.head(top_n).sort_values('count', ascending=True), x='count', y='name',
                             orientation='h', labels={'name': 'Industry', 'count': 'Doc Count'},
